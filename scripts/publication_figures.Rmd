...
environmental_dataset_full <- readr::read_csv("../data/output_data/environmental_dataset_balanced_full.csv") |>
  ...
  dplyr::mutate(
    geometry = purrr::map(geometry, ~ sf::st_point(.x))
  ) |>
...
ggplot2::ggplot(phylo_pca_labelled, ggplot2::aes(x = PC1, y = PC2, color = growth_form)) +
  ...
  ggplot2::scale_color_manual(values = c(
    ...
    "Perennial Herbaceous" = "#ED68ED",
    "Insular Woodiness" = "#00A9FF",
    "Derived Woodiness" =  "#00BE67",
    "Ancestral Woodiness" = "#F8766D"
  )) +
...
```{r}
factoextra::fviz_cos2(phylo_pca_100_25, choice = "var", axes = 1) +
  ggplot2::labs(title = "First PC", y = "")-> p1
factoextra::fviz_cos2(phylo_pca_100_25, choice = "var", axes = 2) +
  ggplot2::labs(title = "Second PC", y = "")-> p2
```
PCA plot with selection of species highlighted:
```{r}
image_dataframe <- data.frame(
  species = c('Mespilodaphne foetens', 'Echium wildpretii', 'Laurus novocanariensis', 'Convolvulus floridus', 'Tamarix canariensis', 'Euphorbia handiensis', 'Viola cheiranthifolia', 'Cistus symphytifolius', 'Adenocarpus foliolosus', 'Bosea yervamora'),
  x = c(5.5, -5.2, 4.2, 1,-1, -5, -4, -1, 2, 3),     # manually set X positions
  y = c(1, -1.5, -3, 4.5, 5, 3.5, -4.5, -4.5, -4, 3.5),       # manually set Y positions
  image = c("../data/plant_images/mespilodaphne_foetens.png",
            "../data/plant_images/echium_wildpretii.png",
            "../data/plant_images/laurus_novocanariensis.png",
            "../data/plant_images/convolvulus floridus.png",
            "../data/plant_images/tamarix_canariensis.png",
            "../data/plant_images/euphorbia_handiensis.png",
            "../data/plant_images/viola_cheiranthifolia.png",
            "../data/plant_images/cistus_symphytifolius.png",
            "../data/plant_images/adenocarpus_foliolosus.png",
            "../data/plant_images/bosea_yervamora.png"),
  size = c(0.35, 0.25, 0.25, 0.25, 0.2, 0.3, 0.3, 0.2, 0.3, 0.2)
)

arrow_dataframe <- merge(
  image_dataframe,
  phylo_pca_500_13_labelled[, c("species", "PC1", "PC2")],
  by = "species"
)
...
highlight_species <- c('Mespilodaphne foetens' = "darkgreen", 
                       ...
                       'Bosea yervamora' = "darkblue")

ggplot2::ggplot(phylo_pca_labelled, ggplot2::aes(x = PC1, y = PC2)) +
  # Segments
  ...
  ggplot2::geom_point(
    data = subset(phylo_pca_labelled, !(species %in% names(highlight_species))),
  ...
  ggplot2::scale_color_manual(values = highlight_species) +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
                y = paste0("PC2 (", round(100 * summary(phylo_pca)$importance[2, 2], 1), "%)"),
                title = "",
                color = 'Growth form',
  ...
  ggplot2::geom_text(x = -3, y = 4.7, label = "Coastal \n specialists", color = "orange", fontface = "bold") +
  ggplot2::geom_text(x = -2.5, y = -3.5, label = "High mountain \n specialists", color = "red", fontface = "bold") +
  ggplot2::geom_text(x = 1, y = 1.8, label = "Thermo-sclerophilous \n specialists", color = "darkblue", fontface = "bold") +
  ggplot2::geom_text(x = 2.8, y = 0.5, label = "Laurisilva specialists", color = "darkgreen", fontface = "bold") +
  ggplot2::geom_text(x = 0.5, y = -3, label = "Pinewood specialists", color = "purple", fontface = "bold")
```
...
ggplot2::ggplot(phylo_pca_labelled_grouped, ggplot2::aes(x = PC1, y = PC2, color = growth_form)) +
  ggplot2::geom_point(size = 3, alpha = 0.8) +
  # Add ellipse:
  ggplot2::stat_ellipse(ggplot2::aes(fill = growth_form), alpha = 0.2, geom = "polygon") +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
...
phylo_pca_labelled |>
  ...
  # Add points:
  ggplot2::geom_point(size = 3, alpha = 0.8, colour = "#ABA300") +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
                y = paste0("PC2 (", round(100 * summary(phylo_pca)$importance[2, 2], 1), "%)"),
                title = "AH",
                color = 'Growth form',
...
phylo_pca_labelled |>
  ...
  ggplot2::stat_density2d(colour = "grey") +
  # Add points:
  ggplot2::geom_point(size = 3, alpha = 0.8, colour = "#ED68ED") +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
                y = paste0("PC2 (", round(100 * summary(phylo_pca)$importance[2, 2], 1), "%)"),
                title = "PH",
                color = 'Growth form',
...
phylo_pca_labelled |>
  ...
  ggplot2::stat_density2d(colour = "grey") +
  # Add points:
  ggplot2::geom_point(size = 3, alpha = 0.8, colour = "#00A9FF") +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
                y = paste0("PC2 (", round(100 * summary(phylo_pca)$importance[2, 2], 1), "%)"),
                title = "IW",
                color = 'Growth form',
...
phylo_pca_labelled |>
  ...
  ggplot2::stat_density2d(colour = "grey") +
  # Add points:
  ggplot2::geom_point(size = 3, alpha = 0.8, colour = "#00BE67") +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
                y = paste0("PC2 (", round(100 * summary(phylo_pca)$importance[2, 2], 1), "%)"),
                title = "DW",
                color = 'Growth form',
...
phylo_pca_labelled |>
  ...
  ggplot2::stat_density2d(colour = "grey") +
  # Add points:
  ggplot2::geom_point(size = 3, alpha = 0.8, colour = "#F8766D") +
  ggplot2::labs(x = paste0("PC1 (", round(100 * summary(phylo_pca)$importance[2, 1], 1), "%)"),
                y = paste0("PC2 (", round(100 * summary(phylo_pca)$importance[2, 2], 1), "%)"),
                title = "AW",
                color = 'Growth form',
...
ggplot2::ggplot() +
  ...
  ggplot2::theme(legend.position = "right", 
                 ...
                 axis.text.x = ggplot2::element_text(size=5.5)) -> p1
...
points_on_island <- environmental_dataset_full |>
  ...
  sf::st_filter(
    shape_file |>
      dplyr::filter(island == "La Palma") |>
      sf::st_set_crs(4326),  # Ensure CRS is set for shape
    .predicate = sf::st_within) |>
  dplyr::left_join(growth_form_list_extended, by = "species") |>
  dplyr::filter(!is.na(growth_form),
                species %in% environmental_data_aggregate_500_13_extended$species) |>
  dplyr::mutate(x = sf::st_coordinates(geometry)[, 1],
                y = sf::st_coordinates(geometry)[, 2])

# 1. Make grid over the island
grid <- island_shape |>
  sf::st_make_grid(cellsize = 300, square = TRUE) |>
  sf::st_as_sf() |>
  sf::st_filter(island_shape, .predicate = sf::st_intersects) |>
  dplyr::mutate(cell_id = dplyr::row_number()) |>
  sf::st_transform(terra::crs(points_on_island))
...
full_cell_summary_all |>
  ...
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
                 ...
                 axis.text.x = ggplot2::element_text(size=5.5)) -> p2
...
full_cell_summary_all |>
  ...
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
                 panel.grid.major = ggplot2::element_blank(),
                 legend.position = "none",
                 axis.text.x = ggplot2::element_text(size=5.5)) -> p3
...
full_cell_summary_all |>
  ...
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
                 panel.grid.major = ggplot2::element_blank(),
                 legend.position = "none",
                 axis.text.x = ggplot2::element_text(size=5.5)) -> p4
...
full_cell_summary_all |>
  ...
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
                 panel.grid.major = ggplot2::element_blank(),
                 legend.position = "none",
                 axis.text.x = ggplot2::element_text(size=5.5)) -> p5
...
full_cell_summary_all |>
  ...
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
                 ...
                 axis.text.x = ggplot2::element_text(size=5.5)) -> p6
